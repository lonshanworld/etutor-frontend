...
const UserBlog = forwardRef<HTMLDivElement, Props>(
  (
    ...
    const [formattedTime, setFormattedTime] = useState(formatTimeStamp(time));
    const {
      commentCounts,
      likeCounts,
      isLiked,
    ...
    useEffect(() => {
      const updateTime = () => setFormattedTime(formatTimeStamp(time));
      updateTime();
      ...
      >
        ...
        <div className='flex py-2 justify-between mx-4 select-none max-md:'>
          <div className='flex gap-10'>
            {/* Like btn */}
            <div
              className={`flex items-center gap-1 cursor-pointer text-secondaryText ${isReadOnly ? "pointer-events-none opacity-50" : "cursor-pointer"}`}
