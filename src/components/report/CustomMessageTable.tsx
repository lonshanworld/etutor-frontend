...
import {
  ChatMessageCount,
  chatMessageCountFromJson,
} from "@/model/chatMessageCount";
...
export default function CustomMessageTable({
      ...
      try {
        ...
        const userList: ChatMessageCount[] = await Promise.all(
          response.users.map(async (item: any) => {
            ...
            const message_count = await getMessageCount({
              senderId: oneUser.id,
              timeRange: duration ?? "",
            });
  ...
  function restartSearch() {
    router.replace(
      `${AppRouter.staffMessage}?page=1&search=${searchInputRef.current?.value ?? ""}&duration=${getDay(selectedTime ?? "")}&type=${getType(selectedType ?? "")}`
    );
  }
        ...
        <div className="w-28">
          <ReportDropDownSelect
            ...
            onChange={(value: string) => {
              ...
              router.replace(
                `${AppRouter.staffMessage}?page=1&search=${search ?? searchInputRef.current?.value ?? ""}&duration=${getDay(selectedTime ?? "")}&type=${getType(value)}`
              );
            }}
          />
        </div>
        <div className="w-28">
          <ReportDropDownSelect
            ...
            onChange={(value: string) => {
              ...
              router.replace(
                `${AppRouter.staffMessage}?page=1&search=${searchInputRef.current?.value ?? ""}&duration=${getDay(value)}&type=${getType(selectedType ?? "")}`
              );
            }}
          />
        </div>
      ...
      >
        <TableHeader className="bg-theme rounded-lg ">
          <TableRow className={`${isSmallScreen === true ? "h-0" : "h-12"}`}>
            ...
            <TableHead className="w-[15%] text-center">Msg Count</TableHead>
          </TableRow>
        </TableHeader>
