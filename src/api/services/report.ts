...
export async function getActiveUsers(currentPage: number): Promise<any> {
  const response = await GetRequest(
    APIS.GET.getReportActiveUsers + `?page=${currentPage}`
  );
...
export async function getUnassignedStudents(
  ...
  const response = await GetRequest(
    APIS.GET.getUnassignedStudents +
...
export async function getAllStudents(
  ...
  const response = await GetRequest(
    APIS.GET.getStudentList + `?page=${currentPage}&search=${search ?? ""}`
  );
  console.log("check list response", response);
  if (isErrorModel(response)) {
    throw response;
...
export async function getChatMessageAll(
  currentPage: number,
  search?: string
): Promise<any> {
  const response = await GetRequest(
    APIS.GET.getChatMessageList + `?page=${currentPage}&search=${search ?? ""}`
  );
  if (isErrorModel(response)) {
    throw response;
  } else {
...
export async function getChatMessageTutor(
  currentPage: number,
  search?: string
): Promise<any> {
  const response = await GetRequest(
    APIS.GET.getTutorList + `?page=${currentPage}&search=${search}`
  );
  if (isErrorModel(response)) {
    throw response;
  } else {
...
export async function getChatMessageStudent(
  currentPage: number,
  search?: string
): Promise<any> {
  const response = await GetRequest(
